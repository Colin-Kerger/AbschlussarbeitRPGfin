...
class Messer (modell: String, dmg: IntRange) : Weapon(modell, dmg) {
    ...
    // hier wird der Schaden der Waffe zu gewiesen
    override fun angriffH(held: Held,gegner: Gegner) {
        var schaden = dmg.random()
        var treffer = Random.nextBoolean()
        if (treffer == true) {

            println("\n$green${held.name}$greenreset wirft $yellow$modell$yellowreset und trifft $blue${gegner.name}$bluereset mit $red$schaden$redreset Schaden!!!\n ")
